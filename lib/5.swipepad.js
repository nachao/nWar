	...
	************************************/
	...
	nSwipepad.fn = nSwipepad.prototype = {
		...
		move: function ( source, target, number ) {
			return nSwipepad.createMovePoint(source, target, number 	)
		}
	};
	...
	$.extend(nSwipepad, {
		...
		// 生成点，并移动到目标位置
		createMovePoint: function ( source, target, number ) {
			...
			for ( var i=0; i<number; i++ ) {
				...
				// 初始化样式
				source.el.find('.swipepad-movePoint').css({
					width: size,
					height: size,
					position: 'absolute',
					borderRadius: '50%',
					...
					backgroundColor: 'black'//source.params.user.color
